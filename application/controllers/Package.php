...
class Package extends CI_Controller
{
	...
	public function index()
	{
		$data = [
			...
			'subcribe'=>$this->db->select('paket.name,subcribe.status,subcribe.amount,subcribe.hash,subcribe.paket')->join('paket','paket.id=subcribe.paket')->order_by('subcribe.id','desc')->get_where("subcribe",['members'=>$this->session->userdata('username')])->row()
